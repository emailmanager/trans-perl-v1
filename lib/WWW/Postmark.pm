package WWW::Emailmanager;
BEGIN {
  $WWW::Emailmanager::VERSION = '0.2';
}
...
# ABSTRACT: API for the Emailmanager mail service for web applications.
...
The WWW::Emailmanager module provides a simple API for the Emailmanager web service,
that provides email sending facilities for web applications. Emailmanager is
located at L<http://emailmanager.com>. It is a paid service that charges
...
recipients limit). If WWW::Emailmanager receives a successful response from
the Emailmanager service, it will return a true value; otherwise it will die.
...
The token is 'EMAILMANAGER_API_TEST', and you can use it for testing purposes
...
Creates a new instance of this class, with a Emailmanager API token that you've
received from the Emailmanager app. By default, requests are made through HTTP;
...
sub new {
	...
	croak "You must provide your Emailmanager API token." unless $token;
...
them. Note, however, that Emailmanager limits this to 20 recipients and sending
...
so you can analyze statistics of your mail sendings through the Emailmanager service.
...
sub send {
	...
	my $req = HTTP::Request->new('post', $self->{use_ssl} ? 'https://trans.emailmanager.com/email' : 'http://trans.emailmanager.com/email', ['Accept' => 'application/json', 'Content-Type' => 'application/json', 'X-Emailmanager-Server-Token' => $self->{token} ], $json->to_json($msg));
...
sub _analyze_response {
	...
	given ($res->code) {
		...
		when (500) {
			return "Emailmanager service error. The service might be down.";
		}
		default {
...
Please report any bugs or feature requests to C<bug-www-emailmanager at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=WWW-Emailmanager>.  I will be notified, and then you'll
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=WWW-Emailmanager>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/WWW-Emailmanager>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/WWW-Emailmanager>

=item * Search CPAN

L<http://search.cpan.org/dist/WWW-Emailmanager/>
